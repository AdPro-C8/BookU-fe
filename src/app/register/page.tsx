...
export default function Home() {
  ...
  const onSubmit = ((data: FormData, e: any) => {
    ...
    axios.post(`${AUTH_API_URL}/auth/signup`, {
      ...
      <div className="bg-black w-1/2 max-w-5xl h-fit m-auto text-center rounded-xl px-5 py-7">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className='relative z-0 w-full mb-6 group text-left'>
            <input className={`block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 focus:outline-none focus:ring-0 ${ errors.username ? `focus:border-red-600 peer dark:focus:border-red-500` : `focus:border-blue-600 peer dark:focus:border-blue-500` }`}
              placeholder=" "
              id="username"
              {...register("username", {
                required: "required",
              ...
              type="text"
            />
            <label htmlFor="username" className={`peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 ${ errors.username ? `peer-focus:text-red-600 peer-focus:dark:text-red-500` : `peer-focus:text-blue-600 peer-focus:dark:text-blue-500` } peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6`}>
              username
            </label>
            {errors.username && <span role="alert" className='text-sm text-red-500'>{errors.username.message}</span>}
          </div>
          <div className='relative z-0 w-full mb-6 group text-left'>
